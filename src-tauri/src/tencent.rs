...
//     }
//   }
//   fn calc_auth(&self, timestamp: i64, body: &str) -> (String, i64) {
//     let hashed_payload = format!("{:x}", Sha256::digest(body.as_bytes()));
...
//     body: String,
//   ) -> anyhow::Result<String> {
//     let (auth, timestamp) = self.calc_auth(timestamp, &body);
//     println!("{}", auth);
...
//     .get()
//     .unwrap()
//     .call_api(region, action, timestamp, body)
//     .await
//     .into_ta_result()
// }
...
//   body: String,
// ) -> TAResult<String> {
//   call_tencent_api(&CVM_CLIENT, region, action, timestamp, body).await
// }
